...
var helpLines = []string{
	...
	"  smart guides snap to other objects' edges/centers (shift off)",
	"  polygon: click vertices, enter or double-click to close",
	"  curve: drag, then drag its round control handles (select tool)",
	"  text: click, type — ←→↑↓ caret, ctrl+j newline, enter commits",
	"  brush strokes are auto-smoothed and simplified on release",
	"  K keyboard-draw mode: arrows move a cursor, enter sets points",
	"",
	"FILES",
	"  ctrl+s save         ctrl+o open     ctrl+e export svg/png",
	"  ctrl+shift+c copy selection as SVG to the system clipboard",
	"  autosaves to <file>.autosave; offers recovery on reopen",
	"",
	"UI",
